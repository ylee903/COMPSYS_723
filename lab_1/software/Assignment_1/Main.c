...
void registerISRs(void)
{
    ...
    /*
     * Keyboard ISR:
     * kept consistent with your conceptual design.
     * If the PS/2 path gives trouble, comment this out first,
     * get the rest compiling/running, then come back to it.
     */
    alt_irq_register(PS2_IRQ, NULL, KeyboardISR);

    /* Enable push-button interrupt mask for all three buttons */
